...
public class HealthCheese : ItemClass
{
    ...
    public HealthCheese()
    {
        _itemName = "HealthCheese";
        _strength = -1;
        _health = 5; // error state, if the thing doesnt change it
        _tempStrength = (-1, -1);
        _tempSpeed = (-1, -1); //if speed, first number is increase by how much, and second is for how long in seconds
        _spriteImage = Resources.Load<Sprite>("Items Pack/Pixel Art Icon Pack - RPG/Texture/Food/Cheese.png");
